        ...
        _render_doc_cards(
            [
                (
                    ...
                    "steps/index",
                    [
                        f"{len(catalog.steps)} step pages with signatures, docstrings,",
                        "parameter details, and real feature examples.",
                    ],
                ),
                (
                    "Type reference",
                    "types/index",
                    [
                        f"{len(catalog.types)} custom type pages with runtime types,",
                        "enum values, and backlinks from steps.",
                    ],
                ),
            ]
        ),
        ...
        _render_doc_cards(
            [
                (
                    ...
                    _step_type_file_name(step_type),
                    [
                        f"{len(grouped_steps[step_type])} step definitions",
                    ],
                )
                for step_type in STEP_TYPES
                if grouped_steps[step_type]
            ...
            _render_doc_cards(
                [
                    (
                        ...
                        type_doc.slug,
                        [
                            f"Runtime type: {type_doc.python_type or 'unknown'}",
                            f"Used by {len(type_doc.used_by_steps)} step(s).",
                        ],
                    )
                    for type_doc in sorted(
                        type_docs.values(),
...
def _render_doc_cards(
    ...
    for title, target, description_lines in cards:
        lines.extend(
            [
